...
describe("reversible native Codex route integration", () => {
  test("explicit setup restores a fully removed hook without discarding current Codex config", () => {
    for (const ending of ["\n", "\r\n"]) {
      ...
      const current = readFileSync(configPath, "utf8").replace(installed.interruptHook.fragment, "");
