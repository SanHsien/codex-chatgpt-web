...
test("workflows package, smoke, and publish Windows assets only", () => {
  ...
  assert.match(release, /path:\s*release-staging/);
  assert.match(release, /releaseAssetsDirectory/);
  assert.match(release, /Get-ChildItem -LiteralPath \$stagingDirectory -Recurse -File/);
  assert.match(release, /expectedProducedAssetNames/);
  assert.match(release, /unexpectedProducedFiles/);
  assert.match(release, /must contain exactly one \$assetName/);
  assert.match(release, /Flat release asset directory must contain exactly the expected Windows payload assets/);
  assert.match(release, /Flat release asset directory must contain exactly the final published asset set/);
  assert.match(release, /finalExpectedAssetNames/);
