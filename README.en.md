...
- **Windows launcher.** The Windows app owns sign-in, model setup, MCP
...
- **Windows only:** release packages target Windows x64. Runtime, tests, and packaging are gated on
  Windows CI; account-bound browser and MCP flows use the
  separate [release validation](docs/release-validation.md).
- Builds are not yet platform-signed, so SmartScreen may warn. The installer verifies
